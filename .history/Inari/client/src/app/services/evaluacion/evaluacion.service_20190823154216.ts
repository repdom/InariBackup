...
export class EvaluacionService extends DataService {
  ...
  filtroDeEvaluaciones(desde: number, tamPagina: number, filtrosWhere: string, filtrosBetween: string) {
   ...
   headers.append('Authorization', this.cookieService);
   const options = new RequestOptions({ headers: headers });
   console.log('Where Activo ' + filtrosWhere);
   // tslint:disable-next-line:max-line-length
   return this.http.get(this.url + `?filter={"limit": "${tamPagina}","skip": "${desde}", "where": {${filtrosWhere}}, "fields": {"codigo": true, "fechaCreacion": true, "fechaGuardado":true, "usuarioRelacionado": true, "formularioModeloCodigo": true, "areaCodigo": true, "bloqueado": true, "liberado": true, "hallazgos": true}, "order": "fechaGuardado DESC"}`, options)
    .pipe(
      map(response => response.json()),
      catchError(this.handlerError)
    );
   ...
   // tslint:disable-next-line:max-line-length
   return this.http.get(this.url + `?filter={"limit": "${tamPagina}", "skip":
    "${desde}", "where": {"fechaGuardado":{"between":${filtrosBetween}},
    "and":[{"or": [{"completado": true}, {"completado":null}]},
    {"or": [{"liberado": true}, {"bloqueado": null}]}]},
    "fields":
      {"codigo": true, "fechaCreacion": true, "fechaGuardado":true, "usuarioRelacionado": true,
      "formularioModeloCodigo": true, "areaCodigo": true, "bloqueado": true, "liberado": true,
      "hallazgos": true}, "order": "fechaGuardado DESC"}`, options)
    .pipe(
      map(response => response.json()),
      catchError(this.handlerError)
    );
   } else if (filtrosBetween === null) {
     console.log('Where null ' + filtrosWhere);
   // tslint:disable-next-line:max-line-length
   return this.http.get(this.url + `?filter={"limit": "${tamPagina}",
   "skip": "${desde}", "where": {"and":[${filtrosWhere}]}, "fields": {"codigo": true, "fechaCreacion": true, "fechaGuardado":true,
   "usuarioRelacionado": true, "formularioModeloCodigo": true, "areaCodigo": true,
   "bloqueado": true, "liberado": true, "hallazgos": true}, "order": "fechaGuardado DESC"}`, options)
    .pipe(
     map(response => response.json()),
     catchError(this.handlerError)
    );
   } else if (filtrosBetween !== null && filtrosWhere !== null) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.url + `?filter={"limit": "${tamPagina}",
    "skip": "${desde}", "where": {"fechaGuardado":{"between":${filtrosBetween}},"and":[${filtrosWhere}]},
    "fields": {"codigo": true, "fechaCreacion": true, "fechaGuardado":true, "usuarioRelacionado": true,
    "formularioModeloCodigo": true, "areaCodigo": true, "bloqueado": true, "liberado": true,
    "hallazgos": true}, "order": "fechaGuardado DESC"}`, options)
    .pipe(
      map(response => response.json()),
  ...
  contarFiltrado(filtrosWhere: string, filtrosBetween: string) {
    const headers = new Headers({ 'Accept': 'application/json', 'Content-Type': 'application/json'});
    headers.append('Authorization', this.cookieService);
    const options = new RequestOptions({ headers: headers });
    if (filtrosWhere === null) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.url + `/count?where={"fechaGuardado":{"between":${filtrosBetween}, "and":[{"or": [{"completado": true}, {"completado":null}]}, {"or": [{"liberado": true}, {"bloqueado": null}]}]}`, options)
     .pipe(
       map(response => response.json()),
       catchError(this.handlerError)
     );
      ...
      console.log('Where null ' + filtrosWhere);
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.url + `/count?where={"and":[${filtrosWhere}]}`, options)
     .pipe(
      map(response => response.json()),
      catchError(this.handlerError)
     );
     ...
     // tslint:disable-next-line:max-line-length
     return this.http.get(this.url + `/count?where={"fechaGuardado":{"between":${filtrosBetween}},"and":[${filtrosWhere}]}`, options)
     .pipe(
       map(response => response.json()),
       catchError(this.handlerError)
     );
