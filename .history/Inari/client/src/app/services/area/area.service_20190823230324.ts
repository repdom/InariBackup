...
export class AreaService extends DataService {
  ...
  getFilterByAttributeOrderByName() {
    ...
    const options = new RequestOptions({ headers: headers });
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.url + `?filter=%7B%22where%22%3A%20%7B%22cancelado%22%3A%200%7D%2C%20%22fields%22%3A%20%7B"usuarioAdministradorArea":true,%22codigo%22%3A%20true%2C%20%22nombre%22%3A%20true%7D%2C%20%20%22order%22%3A%20%22nombre%20DESC%22%7D`, options)
      .pipe(
        map(response => response.json()),
        catchError(this.handlerError)
      );
