...
export class ListarEvaluacionComponent implements OnInit, AfterViewInit {
  ...
  cambiarPagina(pageEvent: PageEvent) {
    ...
    this.bloqueadosService.getFilteredPagination(null, this.tamPagina * pageEvent.pageIndex, this.tamPagina).subscribe(responseEvaluacion => {
  ...
  listarEvaluaciones() {
    this.spinner.show();
    this.bloqueadosService.getFilteredPagination(null, this.indicePagina, this.tamPagina).subscribe(responseEvaluacion => {
      console.log(responseEvaluacion);
      this.spinner.show();
      responseEvaluacion.forEach(elementEvaluacion => {
        // tslint:disable-next-line:prefer-const
