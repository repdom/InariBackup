...
export class ListarEvaluacionComponent implements OnInit, AfterViewInit {
  ...
  buscarFiltro() {
    ...
    if (this.fechaFiltroInicio && this.fechaFiltroFin && !this.filtrosWhere) {
      console.log(JSON.stringify(this.filtrosBetween));
      // this.buscarEvaluacionesConFiltro(0, this.tamPagina, JSON.stringify(this.filtrosBetween), null);
    } else if (!this.fechaFiltroInicio && !this.fechaFiltroFin && this.filtrosWhere) {
      // this.buscarEvaluacionesConFiltro(0, this.tamPagina, null, JSON.stringify(this.filtrosWhere));
    } else if (this.fechaFiltroInicio && this.fechaFiltroFin && this.filtrosWhere) {
      console.log(JSON.stringify(this.filtrosWhere));
      // this.buscarEvaluacionesConFiltro(0, this.tamPagina, JSON.stringify(this.filtrosBetween), JSON.stringify(this.filtrosWhere));
    }
  }

  agregarValorFiltroWhereArea() {
    if (this.codigoArea === 0) {
      this.filtrosBetween.areaCodigo = this.codigoArea;
    }
  }

  agregarValorFiltroWhereSemaforo() {
    if (this.nombreColor === 'Amarillo') {
      this.filtrosWhere.hallazgo = 1;
    } else if (this.nombreColor === 'Rojo') {
      this.filtrosWhere.bloqueado = 1;
    } else if (this.nombreColor === 'Verde') {
      this.filtrosWhere.hallazgo = 0;
      this.filtrosWhere.bloqueado = 0;
    }
    console.log(this.filtrosBetween);
  }
