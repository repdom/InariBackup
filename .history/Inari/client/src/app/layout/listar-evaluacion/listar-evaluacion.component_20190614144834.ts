...
export class ListarEvaluacionComponent implements OnInit, AfterViewInit {
  ...
  cambiarPagina(pageEvent: PageEvent) {
    ...
    if (pageEvent.previousPageIndex > pageEvent.pageIndex || pageEvent.pageSize !== this.tamAnterior) {
    ...
    this.evaluacionService.getFilteredPagination(0, this.acumuladorPagina, this.tamPagina).subscribe(responseEvaluacion => {
          ...
          imprimir += `<thead>
            ...
            evaluacion.itemsEvaluados.forEach(item => {
              imprimir += `<tr>
                              ...
                              <td class="text-center">${(item.evaluacion === 1) ? 'Si' : 'No'}</td>
                              <td class="text-right">
