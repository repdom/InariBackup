...
export class ListarEvaluacionComponent implements OnInit, AfterViewInit {
  ...
  buscarEvaluacionesConFiltro(desde: number, pageIndex: number,tamPagina: number, filtrosWhere: string, filtrosBetween: string) {
    // filtroDeEvaluaciones()
    this.listaEvaluacion = [];
    // tslint:disable-next-line:max-line-length
    this.evaluacionService.getFiltradoPaginacionConHallazgos(0, this.tamPagina * pageIndex, this.tamPagina).subscribe(responseEvaluacion => {
      console.log(responseEvaluacion);
      this.spinner.show();
      responseEvaluacion.forEach(elementEvaluacion => {
        // tslint:disable-next-line:prefer-const
