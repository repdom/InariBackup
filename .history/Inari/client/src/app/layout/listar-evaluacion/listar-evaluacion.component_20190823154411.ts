...
export class ListarEvaluacionComponent implements OnInit, AfterViewInit {
  ...
  filtroConFecha() {
    if (this.fechaFiltroInicio && this.fechaFiltroFin) {
      this.filtrosWhereAny.between.push(this.fechaFiltroInicio, this.fechaFiltroFin);
    }
  }
  ...
  buscarFiltro(pageEvent: PageEvent) {
    ...
    if (this.filtrando === false) {
      this.agregarFiltros();
      this.filtroConFecha();
      this.filtrosWhereAny.and.push({completado: true}, {and: [{or: [{liberado: null}, {liberado: this.opcionFiltroSemaforo.liberado}]}]});
      ...
      /* if (this.fechaFiltroInicio && this.fechaFiltroFin && this.opcionFiltroSemaforo.bloqueado === 0 && this.opcionFiltroSemaforo.hallazgos === 0 && this.opcionFiltroSemaforo.liberado === 0) {
        ...
        // tslint:disable-next-line:max-line-length
        } else if (this.opcionFiltroSemaforo.areaCodigo > 0 && this.opcionFiltroSemaforo.bloqueado === 0
          && this.opcionFiltroSemaforo.hallazgos === 0 && this.opcionFiltroSemaforo.liberado === 0) {
          const filtrarArea = {
            areaCodigo: this.opcionFiltroSemaforo.areaCodigo,
            completado: true
          };
    ...
    if (this.filtrando === false) {
      ...
      /*if (this.fechaFiltroInicio && this.fechaFiltroFin && this.opcionFiltroSemaforo.bloqueado === 0 && this.opcionFiltroSemaforo.hallazgos === 0 && this.opcionFiltroSemaforo.liberado === 0) {
        ...
        this.filtrando = true;
        // tslint:disable-next-line:max-line-length
        // this.buscarEvaluacionesConFiltro(0, this.tamPagina,
        JSON.stringify(this.opcionFiltroSemaforo), JSON.stringify(this.filtrosBetween));
        if (this.opcionFiltroSemaforo.areaCodigo === 0) {
          const filtrarArea = {
            hallazgos: this.opcionFiltroSemaforo.hallazgos,
            bloqueado: this.opcionFiltroSemaforo.bloqueado,
      ...
      /*if (this.fechaFiltroInicio && this.fechaFiltroFin && this.opcionFiltroSemaforo.bloqueado === 0 && this.opcionFiltroSemaforo.hallazgos === 0 && this.opcionFiltroSemaforo.liberado === 0) {
        ...
        if (this.opcionFiltroSemaforo.areaCodigo === 0) {
          ...
          // tslint:disable-next-line:max-line-length
          this.buscarEvaluacionesConFiltro(this.tamPagina * pageEvent.pageIndex, this.tamPagina,
            JSON.stringify(filtrarArea), JSON.stringify(this.filtrosBetween));
        }
      } else if (!this.fechaFiltroInicio && !this.fechaFiltroFin) {
        // tslint:disable-next-line:max-line-length
        this.filtrando = true;
  ...
  limpiarBusqueda() {
    ...
    this.opcionFiltroSemaforo.areaCodigo = 0;
    this.codigoArea = 0;
    this.nombreColor = '';
    this.indicePagina = 0;
    this.tamPagina = 5;
