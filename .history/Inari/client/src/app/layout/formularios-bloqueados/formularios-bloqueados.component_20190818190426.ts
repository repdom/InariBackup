...
export class FormulariosBloqueadosComponent implements OnInit, AfterViewInit {
  ...
  enviarEmailDeCumplidosDosItemImportantes() {
    let cantidad = 0;
    let i = 0;
    cantidadImportantes = this.evaluacion.itemEspeciales.filter(r => r.importante === true).lenght
    this.evaluacion.itemEspeciales.forEach(itemEspeciales => {
      // cantidad +=
  ...
  guardarFormularioBloqueado(evaluacion: Evaluacion) {
    ...
    this.spinner.show();
    let cont = 0;
    if (evaluacion.emailDesbloqueoEnviado === null || evaluacion.emailDesbloqueoEnviado === false) {
      const evaluacionFormulario = {
        codigo: evaluacion.codigo,
        emailDesbloqueoEnviado: true
      };

      this.evaluacionService.update(evaluacionFormulario).subscribe(evaluacionResponseService => {
