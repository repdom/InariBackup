...
export class FormulariosBloqueadosComponent implements OnInit, AfterViewInit {
  ...
  cambiarPagina(pageEvent: PageEvent) {
    ...
    this.bloqueadosService.getFilteredPagination(true, this.cantidadEvaluaciones * pageEvent.pageIndex, this.cantidadEvaluaciones).subscribe(responseEvaluacion => {
  ...
  listarEvaluaciones() {
    this.spinner.show();
    this.bloqueadosService.getFilteredPagination(true, this.indicePagina, this.cantidadEvaluaciones).subscribe(responseEvaluacion => {
      console.log(responseEvaluacion);
      this.spinner.show();
      responseEvaluacion.forEach(elementEvaluacion => {
        // tslint:disable-next-line:prefer-const
