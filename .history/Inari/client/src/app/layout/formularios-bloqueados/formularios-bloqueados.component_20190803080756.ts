...
export class FormulariosBloqueadosComponent implements OnInit, AfterViewInit {
  ...
  compressFile(itemEvaluacion: ItemEvaluacion, i: number) {
    this.imageCompress.uploadFile().then(({image, orientation}) => {
      this.spinner.show();
      this.imgResultBeforeCompress = image;
      console.warn('Size in bytes was:', this.imageCompress.byteCount(image));
      if  (this.imageCompress.byteCount(image) < 720000) {
        itemEvaluacion. = image;
        itemEvaluacion.fotoAreaCargoDesdeDB[i] = false;
        console.warn('Size in bytes is now:', this.imageCompress.byteCount(area.foto[i]));
        console.log(itemEvaluacion.foto[i]);
        this.spinner.hide();
      } else {
        this.imageCompress.compressFile(image, orientation, 50, 50).then(
          result => {
            this.spinner.hide();
            itemEvaluacion.foto[i] = result;
            itemEvaluacion.fotoAreaCargoDesdeDB[i] = false;
            console.warn('Size in bytes is now:', this.imageCompress.byteCount(itemEvaluacion.foto[i]));
            console.log(this.imgResultBeforeCompress);
            console.log(itemEvaluacion.foto[i]);
          }
        );
