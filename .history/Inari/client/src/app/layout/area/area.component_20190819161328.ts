...
import { Component, OnInit, ChangeDetectorRef, ViewChild, AfterViewInit, ApplicationRef } from '@angular/core';
...
export class AreaComponent implements OnInit {
  ...
  compressFile(area: Area, i: number) {
  ...
  actualizarArea(area: Area) {
    ...
    this.areaService.getFromCode(area.codigo).subscribe(areaElement => {
      ...
      this.areaNueva.fotoAreaCargoDesdeDB[0] = (areaElement['foto1'] !== '' || areaElement['foto1'] !== null) ? true : false;
      // tslint:disable-next-line:max-line-length
  ...
  editarArea() {
    ...
    /*let updatePorTextoPlano = '{';
    updatePorTextoPlano += `"codigo": ${this.areaNueva.codigo},`;
    for (let i = 0; i < 10; i++) {
      updatePorTextoPlano += `"foto${i + 1}": ${this.areaNueva.foto[i]},`;
    }
    updatePorTextoPlano += '}';*/
    ...
    this.areaService.update(area).subscribe(areaResponse => {
      ...
      let j = 0;
      this.areaNueva.fotoAreaCargoDesdeDB.forEach(fA => {
        if (fA === false) {
          let updatePorTextoPlano = '{';
          updatePorTextoPlano += `"codigo": ${this.areaNueva.codigo},`;
          // tslint:disable-next-line:max-line-length
          updatePorTextoPlano += `"foto${j + 1}": "${this.areaNueva.foto[j] !== 'https://i.imgur.com/5qqFPl4.png' ? this.areaNueva.foto[j] : ''}"}`;
          this.areaService.updateWithPureString(updatePorTextoPlano, this.areaNueva.codigo).subscribe(responseFoto => {
